...
| __%AppData%__ | Contiene datos de configuración y caché de aplicaciones específicas de cada usuario. |
...
| __System32__ | Subdirectorio de `C:\Windows` que contiene archivos esenciales del sistema y ejecutables críticos. |
| __Temp__ | Directorio temporal donde se almacenan archivos temporales generados por el sistema y aplicaciones, se ubica dentro de `C:\Windows` |
| __Recovery__ | Se encuentra dentro de `System32` y contiene archivos de recuperación del sistema en caso de fallos graves. |
| __Drivers__ | Almacena los controladores de hardware instalados en el sistema y se ubica en `System32`  |
