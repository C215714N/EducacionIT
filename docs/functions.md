...
Las fórmulas son ecuaciones que realizan cálculos, mientras que las funciones son fórmulas predefinidas que ejecutan cálculos específicos utilizando valores de entrada identificados como argumentos. Las funciones simplifican cálculos complejos, y ambas, comienzan con el __signo igual (=)__. Las funciones pueden usarse para ejecutar __operaciones simples o complejas__. 

## Implementacion

1. Ve a la pestaña __Fórmulas__ de la cinta de opciones. 
2. Puedes insertar una funcion de alguna de las siguientes maneras:
    * Haciendo click en _insertar funcion_ y completando los datos de la ventana.
    * Selecciona la _categoría_ en la __biblioteca de funciones__ y elige la funcion correspondiente.
    * Escribiendo en la celda el nombre de la funcion precedido por el signo igual __=FUNCION(`argumentos`)__

## Clasificacion

* __TEXTO:__ Son aquellas que permiten trabajar con los valores de la celda procesandolos como cadenas de texto, indistintamente de su contenido, y pueden utilizarse para dar formato a las celdas, asi como tambien para extraer o concatenar valores.
    | Funcion | Descripcion|
    |-|-|
...
* __NUMERICAS:__ Son aquellas que se utilizan para trabajan con numeros racionales, ignorando en la mayoria de los casos cualquier valor que no sea operativo, y se organizan en diferentes categorias como _operativas, estadisticas, trigonometricas, etc_.
    | Funcion | Descripcion|
    |-|-|
    ...
    |__REDONDEAR(`val`, `n`)__|	redondea el valor con n cantidad de decimales |
...
* __FECHA Y HORA:__ Son aquellas que facilitan la operatoria con los valores almacenados en la _base de datos de fechas_, debido a que la computadora utiliza numeros racionales para obtener las fechas utilizando el entero para los dias y los decimales para las horas.
    | Funcion | Descripcion|
    |-|-|
